  ...
  String lastReadRFID;
  unsigned long RFIDTimer;
  #define RFID_RESET_TIME 1000
  ...
  int getDrawerIndex(uint8_t drawerLocation){
    for (int i = 0; i < NUM_DRAWERS; i++) {
      if (drawerLocations[i] == drawerLocation) {
        return i;
      }
  ...
  void checkRFID() {
    if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
      ...
      if(millis() - RFIDTimer >= RFID_RESET_TIME){
        lastReadRFID = "";
      }
    }
  }
  ...
  cmdMemory commandHandler(){
    if(Serial.available()){
      ...
      if(command.startsWith("LED")){
        ...
        int drawerIndex = getDrawerIndex(drawerLocation);
        delay(100);
        if(drawerIndex >= 0){
          ...
          Serial.println("Invalid Location ID");
        }
      }else {
